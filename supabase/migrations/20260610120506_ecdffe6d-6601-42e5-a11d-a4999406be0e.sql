...
CREATE INDEX IF NOT EXISTS client_subscriptions_email_idx
  ON public.client_subscriptions (lower(customer_email));

-- 2) Trigger de baixa: agora também casa por email
...
BEGIN
  ...
  SELECT p.phone, p.email INTO customer_phone, customer_email
  ...
  WHERE cs.store_id = NEW.store_id
    ...
    AND (
      ...
      OR (customer_phone IS NOT NULL AND cs.customer_phone IS NOT NULL
          AND public.normalize_phone(cs.customer_phone) = public.normalize_phone(customer_phone))
      OR (customer_email IS NOT NULL AND cs.customer_email IS NOT NULL
          AND lower(cs.customer_email) = lower(customer_email))
    )
    AND (
      cs.plan_id IS NULL
  ...
  IF sub_id IS NOT NULL THEN
    ...
    -- Notificação ao cliente quando estiver acabando (1 restante) ou esgotou (0)
...
-- 3) RPC para o cliente logado ver suas assinaturas
