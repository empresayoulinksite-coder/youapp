...
-- 1) Reescreve trigger para também decrementar quando subscription_id já vem setado
...
BEGIN
  ...
  IF NEW.subscription_id IS NOT NULL THEN
    -- Agendamento já vinculado a uma assinatura (ex.: agendado pela assinatura).
    -- Apenas decrementa o crédito.
    sub_id := NEW.subscription_id;
  ELSE
    -- Caminho legado: tenta achar assinatura compatível pelo usuário/telefone/email.
...
-- 2) Catch-up: para cada assinatura, conta quantos bookings concluídos estão vinculados
--    e ajusta services_used se estiver abaixo do total de concluídos vinculados.
WITH counts AS (
  SELECT subscription_id, COUNT(*)::int AS done
  FROM public.bookings
  WHERE subscription_id IS NOT NULL AND status = 'completed'
  GROUP BY subscription_id
)
UPDATE public.client_subscriptions cs
SET services_used = LEAST(c.done, cs.services_total),
    status = CASE WHEN LEAST(c.done, cs.services_total) >= cs.services_total THEN 'expired' ELSE cs.status END,
    updated_at = now()
FROM counts c
WHERE c.subscription_id = cs.id
  AND c.done > cs.services_used;
